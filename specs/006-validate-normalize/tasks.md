...
- [ ] T001 Add runtime deps to `backend/pyproject.toml`: `phonenumbers`, `email-validator`
...
- [ ] T002 Add settings to `backend/src/core/config.py`: `phone_default_region="KZ"`,
...
- [ ] T003 [P] Confirm baselines before any feature work: `cd backend && pytest -m "not slow"` green and
...
- [ ] T004 Create Alembic migration `backend/migrations/versions/0005_validate_normalize.py` adding
...
- [ ] T005 [P] Create `backend/src/models/validation.py`: `ValidationRun` + `ValidationItem` ORM models
...
- [ ] T006 [P] Widen `EDIT_KINDS` in `backend/src/models/triage.py` to `("edit","transliterate","normalize")`
  and update the `StagedEdit.kind` comment (no DDL — column is already `String(16)`).
- [ ] T007 Add `Validation*` Pydantic schemas to `backend/src/api/schemas.py`: `StartValidationBody`,
...
- [ ] T008 [P] Unit test `backend/tests/unit/test_phone_normalizer.py`: KZ/RU national + `+E.164`
  inputs → validity, E.164 string, `mobile` vs unclear (fixed-line / `FIXED_LINE_OR_MOBILE`).
- [ ] T009 [P] Unit test `backend/tests/unit/test_email_validator_service.py`: syntax pass/fail; MX
  present vs absent (monkeypatch `email_validator`'s deliverability/DNS).
- [ ] T010 [P] Unit test `backend/tests/unit/test_website_checker.py` (respx + monkeypatched resolver):
...
- [ ] T011 [P] Unit test `backend/tests/unit/test_region_service.py`: public IP + GeoLite2 → region;
  private/loopback IP or missing DB → `null` (monkeypatch geoip2).
- [ ] T012 [P] Implement `backend/src/services/phone_normalizer.py` (parse/validate/E.164/`number_type`
  → mobile-or-unclear) to pass T008.
- [ ] T013 [P] Implement `backend/src/services/email_validator_service.py` (syntax + MX via
  `email-validator`; classify `invalid_email` vs `dead_email_domain`) to pass T009.
- [ ] T014 [P] Implement `backend/src/services/website_checker.py` (`httpx` sync+async, manual redirect
  ...
  to pass T010.
- [ ] T015 [P] Implement `backend/src/services/region_service.py` (GeoLite2 lookup when `geoip_db_path`
  set + public IP, else `None`) to pass T011.

### Router / worker / frontend scaffolding

- [ ] T016 Create router skeleton `backend/src/api/routers/validation.py` (empty `APIRouter(prefix="/api")`),
  register it in `backend/src/api/routers/__init__.py` and `backend/src/api/main.py`.
- [ ] T017 Create worker skeleton `backend/src/workers/validation_worker.py` with `claim_next_run`
...
- [ ] T018 Add `'tidy'` to `frontend/src/wizard/steps.ts` at **index 6** (label "Tidy", description,
  route `/wizard/tidy`, `prerequisiteKey: 'review'`, `isLongJob: true`, an icon); set **Export** to
  index 7 with `prerequisiteKey: 'tidy'`.
- [ ] T019 Add the `/wizard/tidy` child route (before `export`) → `@/pages/wizard/TidyStep.vue` in
  `frontend/src/router/index.ts`.
- [ ] T020 [P] Create tidy Pinia store skeleton `frontend/src/stores/tidy.ts` (state for current run +
  items + region; empty actions filled in US1/US2).
- [ ] T021 [P] Add validation endpoint stubs + types to `frontend/src/services/api.ts`
...
- [ ] T022 [P] Create `frontend/src/pages/wizard/TidyStep.vue` skeleton + `frontend/src/components/tidy/`
...
- [ ] T023 [P] [US1] Integration test `backend/tests/integration/test_validation_auto.py`: seeded Draft
...
- [ ] T024 [P] [US1] Contract test `backend/tests/contract/test_validation_run_api.py`: `POST
  /working-copies/{id}/validation-runs` (201; 409 when one is active) and `GET /validation-runs/{id}`
  shapes per contract.
- [ ] T025 [P] [US1] Worker test `backend/tests/integration/test_validation_worker.py`: a `queued` run
  is claimed and driven to `completed`; counts populated; resumes if re-claimed.
- [ ] T026 [P] [US1] Frontend test `frontend/tests/tidy-store.spec.ts`: tidy store `startRun` →
  poll → summary counts; undo calls `undoStagedEdit`.

### Implementation for US1

- [ ] T027 [US1] Implement the auto path in `backend/src/services/validation_service.py`: create/claim/
...
- [ ] T029 [US1] Implement `validation_worker.run_once` body: claim a `queued` run and call
  ...
  on unhandled error.
- [ ] T030 [US1] Expose/confirm the staged-edit undo route `POST /api/staged-edits/{id}/undo` (reuse
...
- [ ] T031 [US1] Implement tidy store actions in `frontend/src/stores/tidy.ts`: `startRun(defaultRegion)`,
  `pollRun` (to terminal, reusing the export-poll cadence), `summary`, `undoEdit`.
- [ ] T032 [US1] Build `frontend/src/components/tidy/TidyRunPanel.vue`: start button → running state →
...
- [ ] T035 [P] [US2] Frontend test `frontend/tests/tidy-queue.spec.ts`: queue renders one row per item
  with issue-appropriate actions; resolve/skip/undo call the right api methods and update counts.

### Implementation for US2

- [ ] T036 [US2] Extend `backend/src/services/validation_service.py`: create `ValidationItem` rows for
...
- [ ] T037 [US2] Implement resolution in `validation_service.py`: `resolve_item` (set_type / edit_value
...
- [ ] T038 [US2] Implement item endpoints in `backend/src/api/routers/validation.py`: `GET
  /validation-runs/{id}/items`, `POST /validation-items/{id}/resolve`, `POST /validation-items/{id}/skip`.
- [ ] T039 [US2] Wire `listValidationItems` / `resolveValidationItem` / `skipValidationItem` in
  `frontend/src/services/api.ts` and add queue actions to `frontend/src/stores/tidy.ts`.
- [ ] T040 [US2] Build `frontend/src/components/tidy/TidyQueue.vue` + `TidyQueueItem.vue`: per-issue
...
- [ ] T041 [P] [US3] Frontend test `frontend/tests/steps.spec.ts`: `WIZARD_STEPS` has 7 entries with
  `tidy` between `review` and `export`, and `export.prerequisiteKey === 'tidy'`.
- [ ] T042 [P] [US3] Frontend test `frontend/tests/wizard-tidy.spec.ts`: wizard store derives `tidy`
  `completed`/`running`/`emptyButPassable` from the latest run, `passable('tidy') === true`, and Export
  is gated on Tidy.
- [ ] T043 [P] [US3] Frontend test `frontend/tests/tidy-region.spec.ts`: region detection
...
- [ ] T044 [US3] Extend `frontend/src/stores/wizard.ts`: add `validationRuns` state + `loadTidy()`, and
...
- [ ] T045 [US3] Region setting: `region_service` detect endpoint `GET /api/settings/detect-region` in
...
- [ ] T046 [US3] Passability + lifecycle UX in `TidyStep.vue`/`TidyRunPanel.vue`: Continue-to-Export
...
- [ ] T047 [P] Verify auditability (Principle V): every auto-fix, resolution, and undo writes an
  `audit_service` entry (before→after/why); structured logs redact secrets/PII; `last_error` redacted.
- [ ] T048 [P] Confirm the `worker` container runs the combined loop (no compose change needed since
  `validation_worker` is in `run_all.py`); document the optional `GEOIP_DB_PATH` / website-check env
  knobs in the backend README.
- [ ] T049 [P] Green gates: `cd backend && pytest -m "not slow"`, `cd frontend && npm run lint`
